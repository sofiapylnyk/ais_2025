...
class GraphSearchApp(ttk.Frame):
    ...
    def _handle_state(self,state):
        ...
        elif act=='found': path=state.get('path'); self.append_result(f"Found! length {len(path)} opened {state.get('opened')}"); self.append_result("Path: "+ " -> ".join(map(str,path))); self.draw_graph(path=path, visited=state.get('visited')); self.current_generator=None
        elif act=='not_found': self.append_result(f"Not found opened {state.get('opened')}"); self.draw_graph(visited=state.get('visited')); self.current_generator=None
    ...
    def run_experiments(self):
        ...
        for name,g in variants:
            for algo in ('DFS','BFS'):
                ...
                results.append({'variant':name,'algo':algo,'found':found,'pathlen':pathlen,'opened':opened})
        self.experiments=results; self.append_result("Experiments:\n"); 
        for r in results: self.append_result(f"{r['variant']:10s} | {r['algo']:3s} | found={r['found']} | pathlen={r['pathlen']} | opened={r['opened']}\n")
    ...
    def export_experiments_csv(self):
        ...
        if not path: return
        keys=['variant','algo','found','pathlen','opened']
        try:
